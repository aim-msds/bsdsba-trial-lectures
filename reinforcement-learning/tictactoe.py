...
class State:
    ...
    def getHash(self):
        if self.hashVal is None:
            self.hashVal = str(self.data.reshape(BOARD_SIZE).tolist())
        return self.hashVal
...
class Judger:
    ...
    def play(self, show=False, sleep_time=0):
        self.reset()
...
def play():
    ...
    while continue_play:
        ...
        if winner == player2.symbol:
            ...
            print("Tie!")
        continue_play = str(input('Play again? [Y/N]'))
        continue_play = (False if continue_play.lower() == 'n' else True)
